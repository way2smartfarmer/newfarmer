...
const Product = ({
  ...
  //
  const { compareItems } = useSelector((state) => state.comapre);

  const itemInWishlist = wishlistItems.some((i) => i.product === _id);

  //
  const comaprelist = compareItems.some((i) => i.product === _id);
  ...
  const addToCompare = () => {
    if (comaprelist) {
      ...
      enqueueSnackbar('remove From Compare', { variant: 'success' });
    } else {
      dispatch(addToCompare(_id));
      enqueueSnackbar('Added To Compare', { variant: 'success' });
    }
  };
  ...
  return (
    <div className="flex flex-col items-start gap-2 px-4 py-6 relative hover:shadow-lg rounded-sm">
      ...
      <span
        ...
        className={`${
          comaprelist ? 'text-red-500' : 'hover:text-red-500 text-gray-300'
        } absolute top-6 right-6 cursor-pointer`}
      >
        <FavoriteIcon sx={{ fontSize: '18px' }} />
      </span>
